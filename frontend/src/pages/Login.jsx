...
async function fetchWithRetry(url, options, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    const timer = setTimeout(() => controller.abort(), 95000); // 95 s
    try {
...
export default function Login() {
  // useNavigate kept for the /store link only
  ...
  // navigate() after setUser() can race with Guard reading stale store state,
  // causing a redirect loop back to /login. A hard redirect lets the page
  // re-mount fresh and the store re-hydrates from localStorage correctly.
  function finishLogin(data) {
    saveSession(data);      // write to localStorage FIRST
    setUser(data.user);     // update in-memory store
    window.location.href = "/"; // hard redirect — bypasses the Guard race
  }
  ...
  async function handleOtpSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSuccess("");
        setLoading(false);
        // If session expired, bounce back to credentials
      ...
      // Small pause so user sees the success message, then hard-redirect
    ...
    // No finally setLoading(false) on success path — keeps spinner until redirect
  }
  ...
  // Live connecting message with elapsed seconds
  const connectMsg =
    loading && phase === "credentials" && waitSecs > 3
      ? `Server waking up… ${waitSecs}s elapsed (can take up to 90s on first login)`
      : success;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .vv-btn { width:100%; background:#C9A96E; color:#0e0e0e; border:none; font-family:'Montserrat',sans-serif; font-size:10px; font-weight:600; letter-spacing:3px; text-transform:uppercase; padding:16px; cursor:pointer; transition:background 0.2s,opacity 0.2s; margin-top:8px; }
        ...
        <div className="vv-card">
            ...
            <div className="phase-enter" key="creds">
              ...
              <form onSubmit={handleLogin} autoComplete="off">
                <div className="vv-field">
                  ...
                  <input
                    className="vv-input" type="text" placeholder="username"
                    value={username} onChange={(e) => { setUsername(e.target.value); clearAlerts(); }}
                ...
                <div className="vv-field">
                  ...
                  <div className="vv-input-wrap">
                    <input
                      ...
                      value={password} onChange={(e) => { setPassword(e.target.value); clearAlerts(); }}
              ...
              {/* FIX: use navigate() for /store — this is internal nav, not a post-login redirect */}
          ...
          {phase === "otp" && (
            <div className="phase-enter" key="otp">
              <button
                className="vv-back"
                onClick={() => { setPhase("credentials"); setOtp(["","","","","",""]); setTempToken(""); clearAlerts(); }}
              >
